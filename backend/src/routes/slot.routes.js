...
router.post('/', authenticate, isEntity, (req, res, next) => { req.uploadFolder = 'slots'; next(); }, upload.single('image'), audit('Slot'), ctrl.create);
router.put('/:id', authenticate, isEntity, (req, res, next) => { req.uploadFolder = 'slots'; next(); }, upload.single('image'), audit('Slot'), ctrl.update);
